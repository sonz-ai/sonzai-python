from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, BinaryIO, TextIO, TYPE_CHECKING, Generator

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

from ..types import UNSET, Unset
from typing import cast

if TYPE_CHECKING:
  from ..models.relationship_entry import RelationshipEntry





T = TypeVar("T", bound="RelationshipsResponse")



@_attrs_define
class RelationshipsResponse:
    """ 
        Attributes:
            relationships (list[RelationshipEntry] | None):
            schema (str | Unset): A URL to the JSON Schema for this object.
     """

    relationships: list[RelationshipEntry] | None
    schema: str | Unset = UNSET





    def to_dict(self) -> dict[str, Any]:
        from ..models.relationship_entry import RelationshipEntry
        relationships: list[dict[str, Any]] | None
        if isinstance(self.relationships, list):
            relationships = []
            for relationships_type_0_item_data in self.relationships:
                relationships_type_0_item = relationships_type_0_item_data.to_dict()
                relationships.append(relationships_type_0_item)


        else:
            relationships = self.relationships

        schema = self.schema


        field_dict: dict[str, Any] = {}

        field_dict.update({
            "relationships": relationships,
        })
        if schema is not UNSET:
            field_dict["$schema"] = schema

        return field_dict



    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.relationship_entry import RelationshipEntry
        d = dict(src_dict)
        def _parse_relationships(data: object) -> list[RelationshipEntry] | None:
            if data is None:
                return data
            try:
                if not isinstance(data, list):
                    raise TypeError()
                relationships_type_0 = []
                _relationships_type_0 = data
                for relationships_type_0_item_data in (_relationships_type_0):
                    relationships_type_0_item = RelationshipEntry.from_dict(relationships_type_0_item_data)



                    relationships_type_0.append(relationships_type_0_item)

                return relationships_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(list[RelationshipEntry] | None, data)

        relationships = _parse_relationships(d.pop("relationships"))


        schema = d.pop("$schema", UNSET)

        relationships_response = cls(
            relationships=relationships,
            schema=schema,
        )

        return relationships_response

